...
//
//   POST /api/editor/login   { password, code }                 -> { token, expires }
//   POST /api/editor/logout                                      (Bearer token) revokes it
...
//
// Logging in (password + authenticator code, failed-login limits, alerts): editorAuth.js.
//
// Environment:
//   EDITOR_PASSWORD_HASH, EDITOR_TOTP_SECRET, EDITOR_SESSION_SECRET
//                          login secrets, all printed by `npm run setup-editor`
...
router.use((error, req, res, next) => {
  if (error instanceof HttpError || error instanceof auth.AuthError) return res.status(error.status).json({ error: error.message });
