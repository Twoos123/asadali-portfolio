...
function ProjectItem({ id, image, name, skills, repoUrl }) {
  return (
    <div className="max-w-sm w-full h-[420px] rounded overflow-hidden shadow-lg bg-blue-800 bg-opacity-60 transition-transform duration-300 transform hover:scale-105 flex flex-col">
      <img className="w-full h-48 object-cover" src={image} alt={name} />
      <div className="px-6 py-4 flex-grow flex flex-col">
        <div className="font-bold text-xl mb-2 text-gray-100">{name}</div>
        <p className="text-gray-300 text-lg font-medium flex-grow line-clamp-2">
          {skills}
        </p>
      </div>
